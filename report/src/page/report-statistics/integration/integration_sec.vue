...
<script>
	...
	export default {
		...
		components:{
		 ...
		 data () {		 
            return {
				tHeader:['商家整合','订单数量','商品数量','GMV(万)','销售总额(元)','抵扣总额(元)','实付总额(元)','客单价（元/单）'],
				filterVal:['merchantIntegration','orderNum','goodsAmount','grossMerchandiseVolume','salesTotal','deductionTotal','payAmount','perSales'],
				tHeaderother:['商家整合','订单数量','商品数量','销售总额(元)','抵扣总额(元)','实付总额(元)','客单价（元/单）'],
		...
		methods:{
			getlist(){
				...
				if(this.$store.state.saveData.showDate==1){
					var url=this.$api.integrationSecMonth
				}else if(this.$store.state.saveData.showDate==2){
					var url=this.$api.integrationSecWeek
				}else if(this.$store.state.saveData.showDate==4){
					var url=this.$api.integrationSecOther;
					if(this.$route.query.title=="集团自营"){
						var sendTitle={
							integrationType:1
						}
					}else{
